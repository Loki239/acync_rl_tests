...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        return torch.tensor(np.array(O), dtype=torch.float64, device=self.device),\
            ...
            torch.tensor(np.array(D), dtype=torch.float64, device=self.device)
...
class DDPG:
    def __init__(self, arglist):
        ...
        if hasattr(self.arglist, 'env') and self.arglist.env is not None:
            self.env = self.arglist.env
        elif suite is not None:
            self.env = suite.load(domain_name=self.arglist.domain, task_name=self.arglist.task, task_kwargs={'random': self.arglist.seed})
        else:
            raise RuntimeError("dm_control not found and no env provided in arglist")
    ...
    def train(self):
        ...
        for episode in range(self.start_episode,self.arglist.episodes):
            ...
            while True:
                ...
                self.replay_buffer.push(o, a, r, o_1, done)
                ...
                if done:
                    print(f"Episode {episode}: Reward {ep_r:.2f}")
